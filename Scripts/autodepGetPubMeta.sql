...
WHERE		pub.[publication-date] is not null and
			...
			pub.ID in (  select distinct(pur.[Publication ID])
						  from  [Publication User Relationship] pur, [User] u, [Group] g, [Publication OA Policy] pop
						  where pur.[Publication ID] in (PUB_IDs) and
								...
								pop.[Publication ID] = pur.[Publication ID] and
								pop.[OA Policy ID] = g.[OA Policy ID])
